            ...
            <FormContainer>
                <TabContainer>
                    {tabs.map((label: string, index: number) => (
                        <Tab key={`tab-${index}`} selected={tabIndex === index} label={label} onPress={() => setTabIndex(index)} />
                    ))}
                </TabContainer>
                <Input style={{ marginBottom: 16}} placeholder={tabIndex === 0 ? '전화번호' : '이메일'} />
...
Signup.navigationOptions = {
    header: null,   // stack navigation 에서 제공하는 헤더사용 null(뒤로가기 버튼이 있다.)
}
